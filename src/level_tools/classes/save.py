...
class CCLocalLevels(DictClass):
    
    __slots__ = ("path","levels","lists")
    ...
    def __init__(self, path:str|PathLike=None):
        ...
        with open(path,"w") as file:
            ...
            SaveFile.decrypt_data(file.read())
    ...
    def load(self):
        self.levels = LevelList(*[Level(x) for x in self["LLM_01"]])
        self.lists = LevelList(*[Level(x) for x in self["LLM_03"]])
